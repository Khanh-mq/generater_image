...
sudo apt update -q && sudo apt install aria2 -y -q || apt update -q && apt install aria2 -y -q

# 2. Cài đặt các thư viện Python cần thiết
echo "🐍 [2/3] Đang cài đặt thư viện Python (PyTorch, Diffusers, FastAPI...)..."
pip install --no-cache-dir --break-system-packages -r requirements.txt gdown
